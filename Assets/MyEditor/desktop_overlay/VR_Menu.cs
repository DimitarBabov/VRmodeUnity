...
/*
		...
		if (SteamVR.active)
		{
			...
			GUILayout.BeginHorizontal();
			{
				var t = VR_Camera.sceneResolutionScale;
				...
				if (result != pct)
				{
					VR_Camera.sceneResolutionScale = (float)result / 100.0f;
				}
			}
			GUILayout.EndHorizontal();
		}
	...
	void Update()
	{
		...
		background = monitor.GetComponent<Renderer>().material.mainTexture;
